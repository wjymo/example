...
import com.wjy.paynative.config.SDKConfig;
import com.wjy.paynative.config.UnionpayProperty;
import com.wjy.paynative.constants.SDKConstants;
import com.wjy.paynative.util.AcpService;
import com.wjy.paynative.util.CertUtil;
import com.wjy.paynative.util.DemoBase;
...
public class UnionpayController {
    @Autowired
    private UnionpayProperty unionpayProperty;
    ...
    public void pay(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        String orderId = System.currentTimeMillis()+"xx"; //实际上是orderSn
//        String orderId = request.getParameter("orderId"); //实际上是orderSn

        String txnAmt = null;  //订单金额
        String txnTime = null; //订单发送时间
        //交易金额，单位分，不要带小数点
        BigDecimal amount = BigDecimal.valueOf(0.01);
        amount = amount.multiply(BigDecimal.valueOf(100)).setScale(0, BigDecimal.ROUND_HALF_UP);
        txnAmt = amount.toString();
        //订单发送时间用于查询用
        Date sendPaymentDate = new Date();
        txnTime = FastDateFormat.getInstance("yyyyMMddHHmmss").format(sendPaymentDate);
        ...
        requestData.put("channelType", unionpayProperty.getChannelType());

        /***商户接入参数***/
        requestData.put("merId", unionpayProperty.getMerId());    	          			  //商户号码，请改成自己申请的正式商户号或者open上注册得来的777测试商户号
        ...
        requestData.put("frontUrl", unionpayProperty.getFrontUrl());
        ...
        requestData.put("backUrl", unionpayProperty.getBackUrl());

        // 订单超时时间。
        // 超过此时间后，除网银交易外，其他交易银联系统会拒绝受理，提示超时。 跳转银行网银交易如果超时后交易成功，会自动退款，大约5个工作日金额返还到持卡人账户。
        // 此时间建议取支付时的北京时间加15分钟。
        // 超过超时时间调查询接口应答origRespCode不是A6或者00的就可以判断为失败。
        requestData.put("payTimeout",FastDateFormat.getInstance("yyyyMMddHHmmss").format(new Date().getTime() + 15 * 60 * 1000));
